...
export default function DisclaimerModal() {
  ...
  useEffect(() => {
    localStorage.setItem("dismissed-disclaimer", dismissedDisclaimer.toString());
  }, [dismissedDisclaimer])
	...
	return (
		<AlertDialog open={open}>
          ...
          <ScrollArea className="max-h-[70dvh]">
            ...
            >
              <CollapsibleTrigger
                render={<button type="button" className="flex justify-between w-full cursor-pointer" />}
              >
                <p>How to Add a Course</p>
                <ChevronDown
                  className={clsx("transition-all", { "rotate-180": howToOpen })}
                />
              </CollapsibleTrigger>
              <CollapsibleContent className="mt-2 pt-2 border border-t-border border-transparent">
				...
				<AlertDialogFooter className="justify-between">
					<Field orientation="horizontal">
						<Checkbox id="show-again" checked={dismissedDisclaimer} onCheckedChange={setDismissedDisclaimer} />
						<FieldLabel
							htmlFor="show-again"
