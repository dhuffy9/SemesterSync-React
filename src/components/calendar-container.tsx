			...
			<div className="w-[80px]">
                ...
                </div>
                {
                    Array.from({ length: end - start + 1}).map((_, i) => {
                        ...
                        return (
                            <div key={hour} className="h-25 flex justify-center text-xs text-muted-foreground">
                                {formatHour(hour)}
                            </div>
                        )
				...
				<div className="w-full h-[100px] grid grid-cols-7 border-b">
                    {days.map((day, i) => {
                        ...
                        const isToday = 
                            ...
                            return (
                                <div key={day} className={`${isToday ? "text-primary": ""} flex flex-col justify-center items-center border-r`}>
                                    <span className="text-muted-foreground">
                                        {day}
                                    </span>
                                    <span className={`${isToday ? "bg-primary rounded-full px-2 text-primary-foreground" : ""}`}>
                                        {dataNumber}
                                    </span>
                                </div>
                            )
